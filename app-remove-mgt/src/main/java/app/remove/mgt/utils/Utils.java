...
//        File initialFile = new File("/Users/user/Msc/lcip-lambdas/user-management/src/main/resources/licensekeygenerator.yaml");
        InputStream configInputStream = Utils.class.getClassLoader().getResourceAsStream(fileName);

        //  InputStream configInputStream = Utils.class.getClassLoader().getResourceAsStream(fileName);
        String yamlFileString = null;
        if (configInputStream != null) {
            try (Scanner scanner = new Scanner(configInputStream, StandardCharsets.UTF_8.name())) {
